...
into position_history + trades.csv (Phase 3 v2).
...
v2 NOTE: settlements whose ticker has a prior non-settlement
position_history row will print 'skipped_internal_exit' — this is the
v2 fix that prevents double-attribution. For pre-Phase-2 historical data
(positions exited before 2026-05-02 13:37 UTC) the position_history row
may NOT exist even though the trades.csv exit row does — those settlements
will still be 'inserted' by this script. See the Phase 3 v2 design notes
for the operator decision on whether to also probe trades.csv.
...
async def _run(args: argparse.Namespace) -> int:
    # 1. Build Kalshi auth + client.  Mirrors ep_exec.py wiring.
    auth = (
        NoAuth()
    ...
    # 3. Postgres pool for prior-exit + cost-basis lookups (v2: required;
    #    reconcile_one_settlement returns 'skipped_no_pool' if pool is None).
    ...
    if dsn:
        try:
            ...
            print(f"WARN: Postgres pool init failed ({exc}); v2 prior-exit "
                  "checks will fail-closed (skip every settlement)",
                  file=sys.stderr)
            pool = None
    else:
        print("WARN: DATABASE_URL unset — v2 will skip every settlement "
              "(cannot verify prior internal exits)",
              file=sys.stderr)
    ...
    try:
        ...
        actions = {
            "inserted": 0, "skipped_duplicate": 0, "skipped_internal_exit": 0,
            "skipped_no_pool": 0, "dry_run": 0, "paper_skip": 0, "malformed": 0,
        }
        for s in settlements:
            ...
            print(
                f"{result['action']:22s}  {result['ticker']:32s}  "
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Backfill /portfolio/settlements into position_history (Phase 3 v2).",
    )
    ...
    p.add_argument("--since", default="2026-04-29",
